...
class TennesseeBot extends TrafficBot {
  constructor() {
    super({
      accountName: 'tennessee',
  ...
  async fetchCameras() {
    console.log('Fetching cameras from TDOT...');

    try {
      const response = await Axios.get('https://www.tdot.tn.gov/opendata/api/public/RoadwayCameras', {
        headers: {
      ...
      const cameras = response.data
        .filter(cam => cam.active === 'true' && cam.httpsVideoUrl)
        .map(cam => ({
          id: cam.id,
          name: cam.title || cam.description,
          url: cam.httpsVideoUrl,
          latitude: cam.lat || 0,
          longitude: cam.lng || 0,
        }));
      ...
      console.error('Error fetching Tennessee cameras:', error.message);
      return [];
    }
  }
