...
class IdahoBot extends TrafficBot {
  ...
  async getSession() {
    console.log('Fetching session from 511.idaho.gov...');
    const response = await Axios.get('https://511.idaho.gov/cctv', {
      headers: {
  ...
  async fetchCameras() {
    console.log('Fetching cameras from Idaho DOT...');
    ...
    try {
      ...
      const makeUrl = (query) =>
        `https://511.idaho.gov/List/GetData/Cameras?query=${encodeURIComponent(JSON.stringify(query))}&lang=en-US`;
      ...
      const cameras = data.data
        ...
        .map(cam => {
          ...
          return {
            ...
            url: `https://511.idaho.gov${img.imageUrl}`,
            latitude,
            longitude,
          };
        });
      ...
      console.error('Error fetching Idaho cameras:', error.message);
      return [];
    }
  }
