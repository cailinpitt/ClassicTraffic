...
class NewMexicoBot extends TrafficBot {
  ...
  async fetchCameras() {
    console.log('Fetching cameras from NMRoads...');

    try {
      const response = await Axios.get('https://servicev5.nmroads.com/RealMapWAR/GetCameraInfo', {
        headers: {
      ...
      const data = response.data.cameraInfo || [];

      const cameras = data
        .filter(cam => cam.enabled && cam.stream && cam.sdpFileHighRes)
        .map(cam => ({
          id: cam.name,
          name: cam.title,
          url: `${cam.videoServer}/${cam.sdpFileHighRes}`,
          latitude: cam.lat || 0,
          longitude: cam.lon || 0,
        }));
      ...
      console.error('Error fetching New Mexico cameras:', error.message);
      return [];
    }
  }
  ...
  async downloadVideoSegment(duration) {
    ...
    const captureCmd = `ffmpeg -y -rtmp_live live -t ${duration} -i "${this.chosenCamera.url}" -map 0:v:0 -c copy "${tempPath}"`;
    ...
    const encodeCmd = `ffmpeg -y -i "${tempPath}" -c:v libx264 -preset medium -crf 23 -pix_fmt yuv420p -vf "setpts=0.25*PTS" -an "${this.pathToVideo}"`;
