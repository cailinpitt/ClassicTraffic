...
class MissouriBot extends TrafficBot {
  ...
  async fetchCameras() {
    console.log('Fetching cameras from MoDOT...');

    try {
      const response = await Axios.get('https://traveler.modot.org/timconfig/feed/desktop/StreamingCams2.json', {
        headers: {
          'Referer': 'https://traveler.modot.org/map/',
      ...
      const cameras = response.data
        .filter(cam => cam.html)
        .map(cam => {
          const idMatch = cam.html.match(/MODOT_CAM_(\d+)/);
          return {
            id: idMatch ? idMatch[1] : cam.location,
            name: cam.location,
            url: cam.html,
            latitude: cam.y || 0,
            longitude: cam.x || 0,
      ...
      console.log(`Found ${cameras.length} cameras with HLS streams`);
      return cameras;
    } catch (error) {
      console.error('Error fetching Missouri cameras:', error.message);
      return [];
    }
  }
