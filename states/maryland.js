...
class MarylandBot extends TrafficBot {
  ...
  async fetchCameras() {
    console.log('Fetching cameras from Maryland CHART...');
    ...
    try {
      ...
      const cameras = allCameras
        ...
        .map(cam => ({
          ...
          url: `https://${cam.cctvIp}/rtplive/${cam.id}/playlist.m3u8`,
          latitude: cam.lat || 0,
          longitude: cam.lon || 0,
        }));

      console.log(`${cameras.length} online cameras with HLS streams`);
      return cameras;
    } catch (error) {
      console.error('Error fetching Maryland cameras:', error.message);
      return [];
    }
  }
