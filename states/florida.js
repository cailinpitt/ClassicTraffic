...
class FloridaBot extends TrafficBot {
  ...
  async downloadVideoSegment(duration) {
    const authenticatedUrl = await this.getAuthenticatedVideoUrl(this.chosenCamera.id, this.chosenCamera.url);
    ...
    const captureCmd = `ffmpeg -y -rw_timeout 15000000 -headers "Referer: https://fl511.com/\r\nOrigin: https://fl511.com\r\n" -t ${duration} -i "${authenticatedUrl}" -map 0:v:0 -c copy "${tempPath}"`;
