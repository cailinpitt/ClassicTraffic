...
const numImagesOptions = [150, 300, 450, 600, 750, 900];
...
class PennsylvaniaBot extends TrafficBot {
  ...
  getNumImages() {
    return _.sample(numImagesOptions);
  }
    ...
    try {
      ...
      const cameras = data.data
        .filter(cam => {
          ...
          return img && !img.disabled && !img.blocked && !img.videoUrl;
        })
        .map(cam => {
          ...
          return {
            ...
            url: `https://www.511pa.com${img.imageUrl}`,
            latitude,
            longitude,
          };
        });

      console.log(`${cameras.length} image cameras found`);
      return cameras;
    } catch (error) {
